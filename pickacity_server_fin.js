...
app.get('/', (req, res) => { // 클라이언트가 루트경로에 접속하면 이에 대한 GET요청이 서버에 전달
  ...
  console.log(path.join(__dirname, 'pickacity_server_fin.html')); // path.join() 함수로 현재 스크립트가 실행되는 디렉토리와 파일명을 결합하여 파일의 절대 경로 생성
  res.sendFile(path.join(__dirname, 'pickacity_server_fin.html')); // res.sendFile() 함수로 클라이언트에게 절대 경로 파일 전달
});
